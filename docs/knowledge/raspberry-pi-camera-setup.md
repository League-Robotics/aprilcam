...
## libcamerasrc crashes — and how the bridge survives them

`libcamerasrc` intermittently dies with
`GstLibcameraSrcState::requestCompleted(...): assertion 'wrap->request_.get() ==
request' failed` → **`Caught SIGSEGV`** (seen 1 s–2 min after start, more likely
with two cameras running). The nasty part: gst-launch's fault handler then
prints **"Spinning. Please run 'gdb'…"** and **loops forever instead of
exiting**, so systemd's `Restart=always` never fires (`NRestarts=0`) and the
loopback silently freezes — any client `view`/`tags` on that camera then hangs.

The bridge launcher (`/usr/local/bin/aprilcam-camera-bridge`, generated by the
setup script) defends on three fronts:

1. **`queue max-size-buffers=6 leaky=downstream` right after `libcamerasrc`** —
   gives libcamera its own thread and releases capture requests downstream
   promptly. This is the primary mitigation; with it, both cameras ran 3+ min
   crash-free (vs. crashing within ~2 min without).
2. **Startup stagger (`sleep 4·position`)** — two `libcamerasrc` instances
   initialising simultaneously race in libcamera/PiSP; staggering avoids it.
3. **Watchdog** — gst runs in the background; the script polls the loopback with
   `v4l2-ctl`, and if no frame is readable (the spinning-after-SIGSEGV case) it
   kills gst and exits non-zero so systemd relaunches a fresh pipeline. The
   service sets `StartLimitIntervalSec=0` so the restart loop never gives up.

If a CSI `view` ever hangs, check `journalctl -u 'aprilcam-bridge@*'` for
`SIGSEGV`/`watchdog` lines and `v4l2-ctl -d /dev/video7N --stream-mmap
--stream-count=5` to see whether frames are flowing.
